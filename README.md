...
[BenchEcho] Report, 256 KiB payloads

The same 10,000 connections with one 262,144-byte message in flight on
each, 200,000 echoes, echo only. The loopback link is the ceiling at about
4.2 GB/s each way, so the columns that separate are CPU and memory; TP99
is the link's queue, not the servers. nhooyr is missing because its server
here keeps coder/websocket's default 32 KiB read limit.
